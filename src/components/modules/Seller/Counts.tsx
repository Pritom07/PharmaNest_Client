...
import { getMedicineCountData } from "@/actions/medicine.action";
import { getActiveRevenue } from "@/actions/orderItem.action";
...
const Counts = () => {
  ...
  const [medicines_lowStock, setMedicines_LowStock] = useState<{
    added_Medicines: number;
    low_Stock: number;
  }>();

  useEffect(() => {
    (async () => {
      const { data } = await getActiveRevenue();
      setActiveRevenue(data?.data);
    })();

    (async () => {
      const { data } = await getMedicineCountData();
      setMedicines_LowStock(data?.data);
    })();
  }, []);
    ...
    <div className="pt-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
        <div className="bg-white shadow-xl rounded-2xl p-10 border-2 border-green-600">
          ...
          <h1 className="flex justify-center items-center gap-3 text-3xl font-bold mt-2 text-green-600">
            <TbCoinTaka /> {activeRevenue?.total_Revenue}
          </h1>
        </div>
